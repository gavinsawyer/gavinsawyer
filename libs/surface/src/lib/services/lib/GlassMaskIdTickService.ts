...
export class GlassMaskIdTickService {
  ...
  public readonly glassMaskIdTick$: Signal<0 | 1> = isPlatformBrowser(this.platformId) ? toSignal(
    this.tickedSubject.asObservable().pipe<void | number | undefined, 0 | 1, 0 | 1>(
      mergeWith<void, [ void ]>(
        new Observable<void>(
